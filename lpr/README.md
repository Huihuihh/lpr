...
* Configuration file: [config.py](chinese_lp/config.py).
...
```Bash
python3 export.py --data_type FP32 --output_dir model/export chinese_lp/config.py
```

**Default export path**:
`model/export_<step>/frozen_graph` - path to the frozen graph
`model/export_<step>/IR/<data_type>` - path to the model converted to the IR format
...
`chinese_lp/config.py` and must look like a text file
...
input data by running `chinese_lp/infer.py`:

```Bash
python3 infer_checkpoint.py chinese_lp/config.py
```

### For a Frozen Graph

```Bash
python3 infer.py --model model/export/frozen_graph/graph.pb.frozen \
...
```Bash
python3 infer_ie.py --model model/export/IR/FP32/lpr.xml \
  --device=CPU \
